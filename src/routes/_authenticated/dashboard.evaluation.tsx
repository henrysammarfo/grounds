...
function EvaluationPage() {
  const { metrics, accuracyByCase, workspace } = useWorkspace();
  ...
  const kpis = [
    ...
    },
    {
      ...
      baseline: `${metrics.humanMinutes.baseline.toFixed(1)}m`,
      grounds: `${metrics.humanMinutes.grounds.toFixed(1)}m`,
      delta: `${(metrics.humanMinutes.grounds - metrics.humanMinutes.baseline).toFixed(1)}m`,
      good: true,
    },
    {
      icon: DollarSign,
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="t-meta mt-2">
          {workspace.workspaceName} · {metrics.casesRun} cases · {metrics.source}
        </p>
      </div>

      {!metrics.casesRun ? (
        <div className="panel p-10 text-center">
          <p className="t-item">No scored packs yet</p>
          <p className="t-caption mt-2">
            Create a pack and Start run — scores stay private to this account.
          </p>
        </div>
      ) : (
        <>
          ...
          <section className="panel p-6">
            ...
            <p className="t-caption mt-1">Baseline (grey) vs GROUNDS (accent)</p>
          ...
          <section className="panel flex flex-wrap items-start gap-4 p-6">
            ...
            <div className="max-w-[70ch]">
              <p className="t-item">Tenant isolation</p>
              <p className="t-body mt-2">
                These numbers belong to {workspace.workspaceName}. Creating another account starts
                a blank workspace — packs and scores are not shared.
              </p>
            </div>
          </section>
        </>
